...
const FROM_ADDRESS = process.env.EMAIL_FROM || 'Money IntX <hello@moneyintx.com>';
...
async function sendReminderEmail({ to, fromName, fromEmail, message, invoiceNumber, amount, totalAmt, paidAmt, viewUrl, siteUrl, appName, tagline, txType, isSelf, isReminder, entryStatus, logoData, currency }) {
  ...
  // Direction-aware subject + heading — uses app terminology (I Owe Them / They Owe Me)
  // isSelf: when true, this email is the sender's own confirmation copy
  let subject, heading, subheading, amtLabel;
  if (txType === 'you_owe_them') {
    // Sender owes the recipient → recipient sees "They Owe Me"
    subject    = `They Owe Me — record from ${fromName}`;
    heading    = 'They Owe Me';
    ...
    // Sender paid the recipient → recipient sees "They Settled Me"
    ...
    // Recipient settled their balance with sender → recipient sees "I Settled Them"
    ...
    // they_owe_you — recipient owes sender → recipient sees "I Owe Them"
    const isPartial = (entryStatus === 'partially_settled');
    subject    = isPartial
      ? `Partial balance still due — reminder from ${fromName}`
      : `I Owe Them — record from ${fromName}`;
    heading    = 'I Owe Them';
    subheading = isPartial
      ? `<strong>${fromName}</strong> has recorded a partial settlement. A balance is still outstanding.`
  ...
  // Self-notification: override subheading to sender's own perspective
  // fromName is the contact name (flipped by client), so use it as the "to" party
  if (isSelf) {
    if (txType === 'you_owe_them') {
      // Self flipped from they_owe_you → "They Owe Me" confirmation
      subject    = `They Owe Me — your record for ${fromName}`;
      subheading = `You have recorded that <strong>${fromName}</strong> owes you this amount.`;
    } else if (txType === 'you_paid_them') {
      subject    = `They Settled Me — your record for ${fromName}`;
      subheading = `You have recorded that <strong>${fromName}</strong> made a payment to you.`;
    } else if (txType === 'they_paid_you') {
      subject    = `I Settled Them — your record for ${fromName}`;
      subheading = `You have recorded that you settled a balance with <strong>${fromName}</strong>.`;
    } else if (txType === 'invoice' || txType === 'bill') {
      const docType = txType === 'invoice' ? 'Invoice' : 'Bill';
      subject    = `${docType} — your record for ${fromName}`;
      subheading = `You have sent a ${docType.toLowerCase()} to <strong>${fromName}</strong>.`;
    } else {
      // they_owe_you flipped → "I Owe Them" confirmation
      subject    = `I Owe Them — your record for ${fromName}`;
      subheading = `You have recorded that you owe <strong>${fromName}</strong> this amount.`;
    }
  }

  // Prefix "Reminder:" for follow-up reminders (not initial notifications)
  if (isReminder && !isSelf) {
    subject = 'Reminder: ' + subject;
    heading = 'Reminder — ' + heading;
  } else if (isReminder && isSelf) {
    subject = 'Reminder Sent: ' + subject;
    heading = 'Reminder Sent — ' + heading;
  }
...
// ── Asset Locker OTP (owner verifies identity to view locked records) ────────
async function sendLockerOtpEmail({ to, fromName, code, siteUrl, appName, tagline, logoData }) {
  const siteName    = appName  || 'Money IntX';
  const siteTagline = tagline  || 'Making Money Matters Memorable';
  const siteBase    = siteUrl  || 'https://moneyinteractions.com';
  const fromAddr    = `${siteName} <${FROM_DOMAIN}>`;
  ...
  return sendEmail({
    to, from: fromAddr,
    subject: `Your Asset Locker Access Code — ${siteName}`,
    html: `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;background:#f9f9f9;">
      <div style="background:#080b53;padding:20px 32px 16px;border-radius:12px 12px 0 0;text-align:center;">
        <img src="${_escHtml(validLogoUrl)}" alt="${_escHtml(siteName)}" style="height:48px;max-width:180px;object-fit:contain;display:block;margin:0 auto 8px;" onerror="this.style.display='none'">
      </div>
      <div style="background:#fff;padding:32px 32px 24px;border-left:1px solid #eee;border-right:1px solid #eee;">
        <h2 style="margin:0 0 6px;font-size:22px;color:#080b53;">Asset Locker Access Code</h2>
        <p style="color:#555;font-size:14px;margin-bottom:24px;">You requested access to your Asset Lockers on ${_escHtml(siteName)}. Enter the code below to unlock your records for this session.</p>
        <div style="text-align:center;margin:0 0 28px;">
          <div style="display:inline-block;background:#f5f5ff;border:2px solid #6c63ff;border-radius:12px;padding:22px 40px;">
            <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#6c63ff;margin-bottom:10px;">Your Access Code</div>
            <div style="font-size:40px;font-weight:800;letter-spacing:10px;color:#080b53;font-family:monospace;">${_escHtml(code)}</div>
      ...
      <div style="background:#f0f0f0;padding:16px 32px;border-radius:0 0 12px 12px;border:1px solid #eee;border-top:none;text-align:center;">
        <p style="margin:0 0 4px;font-size:12px;color:#888;">${_escHtml(siteName)} — ${_escHtml(siteTagline)}</p>
        <p style="margin:0;font-size:11px;color:#bbb;">This code is for identity verification only and does not transfer money or data.</p>
      </div>
    </div>`,
    text: `Asset Locker Access Code\n\nYour one-time code: ${code}\n\nThis code expires in 30 minutes. Enter it in the app to unlock your Asset Lockers for this session.\n\nIf you did not request this, your lockers remain locked.\n\n— ${siteName}`
  });
}

// ── Asset Locker Info (send specific locker details to a named trustee) ───────
async function sendLockerInfoEmail({ to, trusteeName, fromName, lockerTitle, lockerType, lockerLocation, lockerAccess, lockerKey, lockerNotes, siteUrl, appName, tagline, logoData }) {
  const siteName    = appName  || 'Money IntX';
  const siteTagline = tagline  || 'Making Money Matters Memorable';
  const siteBase    = siteUrl  || 'https://moneyinteractions.com';
  const fromAddr    = `${siteName} <${FROM_DOMAIN}>`;
  ...
  return sendEmail({
    to, from: fromAddr,
    subject: `Asset Locker: ${lockerTitle} — ${siteName}`,
    html: `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;background:#f9f9f9;">
      <div style="background:#080b53;padding:20px 32px 16px;border-radius:12px 12px 0 0;text-align:center;">
        <img src="${_escHtml(validLogoUrl)}" alt="${_escHtml(siteName)}" style="height:48px;max-width:180px;object-fit:contain;display:block;margin:0 auto 8px;" onerror="this.style.display='none'">
      </div>
      <div style="background:#fff;padding:32px 32px 24px;border-left:1px solid #eee;border-right:1px solid #eee;">
        <h2 style="margin:0 0 6px;font-size:20px;color:#080b53;">Asset Locker Information</h2>
        <p style="color:#555;font-size:14px;margin-bottom:24px;">This information has been sent to you by <strong>${_escHtml(fromName)}</strong> via ${_escHtml(siteName)}. You have been designated as a trustee for the following asset.</p>
        <div style="background:#f5f5ff;border-left:4px solid #6c63ff;border-radius:4px;padding:16px 20px;margin-bottom:24px;">
    ...
    text: `Asset Locker Information\n\nSent by: ${fromName}\nLocker: ${lockerTitle}\n\n${rows.map(([k,v])=>k+': '+v).join('\n')}\n\nThis information does not transfer ownership or constitute a legal instruction.\n\n— ${siteName}`
  });
}

module.exports = { sendEmail, sendPasswordReset, sendInvoiceEmail, sendReminderEmail, sendInviteEmail, sendNokVerificationEmail, sendNokActivationEmail, sendLockerOtpEmail, sendLockerInfoEmail };
